...
import { type ActivitiesGuideData, type LearningUnit } from "@/lib/planning/activities-guide";
...
const ACT_COLS = [
  { key: "title", label: "TÍTULO DE LA ACTIVIDAD", width: "16%" },
  { key: "instructions", label: "INSTRUCCIONES", width: "21%" },
  { key: "materials", label: "MATERIALES O RECURSO", width: "17%" },
  { key: "participation", label: "FORMA DE PARTICIPACIÓN INDIVIDUAL O COLABORATIVA", width: "15%" },
  { key: "midTerm", label: "MEDIO & TIEMPO DE ENTREGA", width: "15%" },
  { key: "weight", label: "PONDERACIÓN DE ACTIVIDAD & CRITERIOS DE EVALUACIÓN", width: "16%" },
] as const;
...
function Unit({ unit }: { unit: LearningUnit }) {
  const x = (v: boolean) => (v ? "X" : "");
  return (
    <div style={{ marginBottom: 28, breakInside: "avoid" }}>
      <table style={{ width: "100%", borderCollapse: "collapse", tableLayout: "fixed" }}>
        <tbody>
          {metaRow("NOMBRE DE LA UNIDAD DE APRENDIZAJE", unit.name)}
          {metaRow("OBJETIVO ESPECÍFICO DE LA UNIDAD", unit.objective)}
          {metaRow("PERIODO DE REALIZACIÓN DE ACTIVIDADES", unit.period)}
          {metaRow("PONDERACIÓN GENERAL DE ACTIVIDADES", unit.generalWeight)}
          <tr>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", tableLayout: "fixed" }}>
                <tbody>
                  <tr>
                    ...
                    <td style={{ ...cellBase, fontWeight: 700, width: "25%", border: "none", borderRight: `1px solid ${BORDER}` }}>
                      CONOCIMIENTOS: <span style={{ fontWeight: 400, color: TEXT }}>{x(unit.criteria.knowledge)}</span>
                    </td>
                    <td style={{ ...cellBase, fontWeight: 700, width: "25%", border: "none", borderRight: `1px solid ${BORDER}` }}>
                      HABILIDADES: <span style={{ fontWeight: 400, color: TEXT }}>{x(unit.criteria.skills)}</span>
                    </td>
                    <td style={{ ...cellBase, fontWeight: 700, width: "25%", border: "none" }}>
                      ACTITUDES: <span style={{ fontWeight: 400, color: TEXT }}>{x(unit.criteria.attitudes)}</span>
                    </td>
                  </tr>
                </tbody>
              </table>
        ...
        <tbody>
          {unit.activities.length === 0 ? (
            ...
            unit.activities.map((a) => (
              <tr key={a.id}>
                <td style={cellBase}>{a.title}</td>
                <td style={cellBase}>{a.instructions}</td>
                <td style={cellBase}>{a.materials}</td>
                <td style={cellBase}>{a.participation}</td>
                <td style={cellBase}>{a.midTerm}</td>
                <td style={cellBase}>{a.weight}</td>
              </tr>
            ))
          )}
        </tbody>
...
export function ActivitiesGuideDocument({ data }: { data: ActivitiesGuideData }) {
  return (
      ...
      <div style={{ position: "relative", marginBottom: 20, minHeight: 56 }}>
        ...
        <div style={{ textAlign: "center", padding: "0 64px" }}>
          ...
          {data.courseName.trim() ? (
            <p style={{ fontSize: 12, fontWeight: 700, color: PURPLE_SOFT, margin: "8px 0 0" }}>{data.courseName}</p>
          ) : null}
        </div>
      </div>
