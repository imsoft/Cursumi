...
export const RegisterForm = ({ returnUrl, googleAuthEnabled = false, referralCode }: RegisterFormProps) => {
  ...
  const onSubmit = async (values: RegisterFormValues) => {
    try {
      ...
      // Enviar el token de Turnstile como campo adicional junto al registro
      ...
      if (result.error) {
        // Mensaje genérico: no revelar si el email ya está registrado ni otros detalles.
        // El servidor normaliza USER_ALREADY_EXISTS a 200 (ver auth.ts hooks.after),
        // por lo que este bloque solo se ejecuta ante errores técnicos reales.
