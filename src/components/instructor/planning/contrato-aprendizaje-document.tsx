...
import type { ContratoAprendizajeData, CompromisoRow } from "@/lib/planning/contrato-aprendizaje";
...
function CompromisosTable({ rows }: { rows: CompromisoRow[] }) {
  const visibles = rows.filter((r) => r.descripcion.trim());
  return (
...
export function ContratoAprendizajeDocument({ data }: { data: ContratoAprendizajeData }) {
  return (
    <div
      style={{
        width: 794,
    ...
    >
      ...
      <CompromisosTable rows={data.compromisosFacilitador} />

      {/* 2. Compromisos del participante */}
      <div style={{ fontWeight: 700, fontSize: 12, marginTop: 18, color: TEXT }}>
        2. Compromisos del participante
      </div>
      <CompromisosTable rows={data.compromisosParticipante} />
      ...
      <div style={{ display: "flex", justifyContent: "space-between", marginTop: 64 }}>
        ...
        <SignatureLine
          ...
          name={data.nombreInstructor || undefined}
        />
      </div>
    </div>
  );
